...
procedure Evaluer_Alea_TH is
	...
	is
		package Mon_Alea is
			new Alea (1, Borne);
		use Mon_Alea;
                function Hachage_identite(Cle: Integer) return Integer is
                begin
                     return Cle;
                end Hachage_identite;

                package th_integer is
                      new TH(T_Cle=>integer,T_Donnee=>integer,Capacite=>taille,fonction_hachage=>Hachage_identite);
                use th_integer;
        ...
        begin
                      Initialiser_th(Th);
                          ...
                          -- Initialise les fr�quences
                          for i in 1..Borne loop
                                  Enregistrer_th(Th, i, 0);
                          end loop;

                          for i in 1..Taille loop
                               Get_Random_Number(Nombre_alea);
                               Enregistrer_th(Th, Nombre_alea, La_Donnee_TH(Th, Nombre_alea) + 1);
                          end loop;
