/** (mini)Projet (mini)Shell
 ** Développer un interpréteur de commandes simplifié
 ** @author : Abdelmalek rhayoute
 */
 #define YEL "\e[0;33m"
 #define RESET "\e[0;27m"
  #define RED "\e[0;31m"
#include <stdlib.h> /* EXIT_SUCCESS */
#include <stdio.h>  /* entrées sorties: printf ... */
#include <unistd.h> /* pimitives de base : fork, ...*/
#include <string.h> /* opérations sur les chaînes */
#include "readcmd.h"  /* lecture des commandes */
#include <sys/wait.h> /* wait */
#include <errno.h>   /* errno */
#include <fcntl.h>    /* Files operations */
#include <signal.h>   /* traitement des signaux */
#include "job.c" //Contient tout ce qui est relatif au stockage des processus, leur informations ainsi que leur affichage

/** Variables Globales. */    
pid_t pid ;// Le process fils.
int main(int argc, char *argv[]) {

   struct cmdline *cmdl; // la structure qui contient la ligne de commanade      
    /* Boucle Infini qui décrit le comportement de base de l'interpréteur */
    while (1){
         printf( YEL "arhayout@Minishell:~$ " RESET);//indiquer que c'est un mini-shell     
        fflush(stdout);//vider la mémoire tampon et forçer l'affichage
        /* Lire la commande. */
        cmdl = readcmd();   
        pid = fork();
        
        
        if (pid < 0) {   /* échec du fork */
                   perror("fork");
                }
         /* fils */
        else if (pid == 0) {
        /*Q11. tupes complexes.*/
            if (cmdl->seq[1]!= NULL) {
                        int i = 0; //nombre de pipes -1
                        /*calcul nombre de pipes -1*/
                        while (cmdl->seq[i+1] != NULL){
                             i++;
                        }
                        /* Descripteur de pipe.*/
                        int fds[2*i] ;  
                        for (int j = 0; j< i; j++) {
                            if (pipe(fds +j*2) < 0) {
                                perror("erreur pipes");
                                exit(1);
                            }
                        }
                        int j = 1;
                        int child;
                        child = fork() ;
                        /* First commande*/

                        if (child < 0) {
                                printf("Erreur fork\n") ;
                                exit(1) ;
                            }
/*EXECUTION DE LA PREMIÈRE COMMMANDE*/
                            else if (child == 0) {
                            close(fds[0]);
                            /*dup2 function creates a copy of the given file descriptor and assigns a new integer to it*/
                            if(dup2(fds[1],1)){
                            printf("Erreur dup2");
                            }
                            execvp((cmdl->seq)[0][0],(cmdl->seq)[0]) ;    
                            }
                            else {
                                int status;
                                waitpid(child,&status,WUNTRACED);
                                }

/*EXÉCUTION DES COMMANDES INTERMÉDIARES*/
                        while (j < i) {
                            int descriptor1,descriptor2;
                            child = fork() ;
                            if (child < 0) {
                                printf("Erreur fork\n") ;
                                exit(1) ;
                            }
                            else if (child == 0) {
                                /*dup2 function creates a copy of the given file descriptor and assigns a new integer to it*/
                                //rediriger stdout vers l'entrée de la commande suivante
                                 descriptor1 = dup2(fds[2*j-1],0) ;
                                if(descriptor1==-1){
                                printf("Erreur dup2");
                                exit(1) ;
                                }
                                execvp((cmdl->seq)[j][0],(cmdl->seq)[j]) ;
                            } 
                            else {
                                    //rediriger stdin vers le résultat de la commande précédente
                                    descriptor2=dup2(fds[2*j+1],1);
                                    if (descriptor2 == -1) { 
                                    printf("Erreur dup2\n") ;
                                    exit(1) ;
                                    }
                            }
                            j++;
                        }

/*EXECUTION DE LA DERNIÈRE COMMANDE*/
                            child = fork() ;
                            if (child < 0) {
                                printf("Erreur fork\n") ;
                                exit(1) ;
                            }
                            else if (child == 0) {
                                        int status;
                                waitpid(0,&status,WUNTRACED);
                                close(fds[2*j-1]) ;
                                if (dup2(fds[2*j-2], 0) == -1) { 
                                printf("Erreur dup2\n") ;
                                exit(1) ;
                                }
                                close(fds[2*j-2]);
                                execvp((cmdl->seq)[j][0],(cmdl->seq)[j]);
                                perror("erreur exec");
                                exit(1);
                            }
                            else {
                                
                                int status;
                                waitpid(0,&status,WUNTRACED);
                                }
                             
            //fermer les pipes inutiles
                                for (int k = 0; k < 2*i; k++)
                                {
                                    close(fds[k]);
                                }
            }
            else{
                printf( RED "Ce programe est un test de la question 11.\n");
            }
        }
        else{
        /*Pointer to an area where status information about how the child process ended is to be placed.*/
        int status;
        /**f status information is not available, waitpid returns a 0 . WUNTRACED. causes the call to waitpid
        **to return status information for a specified process that has either stopped or terminated. Normally,
        **status information is returned only for terminated processes.
        */
        waitpid(pid,&status,WUNTRACED);
        }
    }
    return EXIT_SUCCESS;
}        



